...
public class RestfulController {
	...
	public Map<String, Object> queryTableLists(@RequestBody @ApiParam(name="数据库连接对象",value="传入json格式",required=true) ConnectionParams param) {
		...
		try {
			if(dbTypeMapper.containsKey(param.getType())) {
				...
				for (Map.Entry<String, String> entry : ret.entrySet()) {
					...
					item.put("tableType", entry.getValue());
					data.add(item);
				}
				return success(data);
			}else {
	...
	@ApiOperation(value="获取表的信息", notes="根据指定的数据库连接信息获取表的信息")
	 public Map<String, Object> queryTableInfo(@RequestBody @ApiParam(name="数据库连接对象与表名称",value="传入json格式",required=true) QueryTableParams param) {
		...
		try {
			if(dbTypeMapper.containsKey(param.getType())) {
				...
				String sourceTableName=param.getTableNameSrc();
				String targetTableName=param.getTableNameDest();

				Map<String, Object> ret=new HashMap<String, Object>();
				ret.put("primaryKeys", dmt.querySourceTablePrimaryKeys(sourceTableName));
				ret.put("columnFileds", dmt.querySourceTableColumnDesc(sourceTableName));
				ret.put("createTableSql", dmt.getMysqlCreateTableSql(sourceTableName, targetTableName, true));
				return success(ret);
			}else {
				throw new RuntimeException("Unsuppot database type string");
			}
